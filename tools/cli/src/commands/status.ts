...
export const statusCommand = new Command("status")
  ...
  .action(() => {
    ...
    // Summary
    const runningCount = Object.values(containerStatus).filter(
      (s) => s.running,
    ).length;
    ...
    if (runningCount === totalCount) {
      console.log(
        chalk.green(`✓ All services running (${runningCount}/${totalCount})`),
      );
    } else if (runningCount > 0) {
      console.log(
        chalk.yellow(`⚠ Some services running (${runningCount}/${totalCount})`),
      );
    } else {
