...
export const execCommand = new Command("exec")
  ...
  .action(async (service?: string, commandArgs: string[] = []) => {
    let selectedService = service
      ? serviceRegistry.resolveServiceName(service)
      : undefined;
    ...
    if (!selectedService || !serviceRegistry.hasService(selectedService)) {
      ...
      printError(
        `Valid services: ${serviceRegistry.getServiceNames().join(", ")}`,
      );
      process.exit(1);
    }
    ...
    if (!serviceConfig?.capabilities.canExec) {
      printError(
        `Cannot exec into ${selectedService} - not an execable service`,
      );
      printError(
        `Execable services: ${execableServices.map((s) => s.name).join(", ")}`,
      );
      process.exit(1);
    }
