  ...
  .action(async (service) => {
    ...
    if (resolvedService === "all") {
      ...
      if (!serviceConfig) {
        ...
        printError(
          `Valid services: ${rebuildableServices.map((s) => s.name).join(", ")}, all`,
        );
        process.exit(1);
      }
      ...
      if (!serviceConfig.capabilities.canRebuild) {
        ...
        printError(
          `Rebuildable services: ${rebuildableServices.map((s) => s.name).join(", ")}`,
        );
        process.exit(1);
      }
      ...
      await runCommandAsync(
        `docker compose --profile dev stop ${resolvedService}`,
      );
      await runCommandAsync(
        `docker compose --profile dev rm -f ${resolvedService}`,
      );

      const spinner = createSpinner(`Building ${displayName}...`).start();
      const buildResult = await runCommandAsync(
        `docker compose --profile dev build --no-cache ${resolvedService}`,
      );
      ...
      const upResult = await runCommandAsync(
        `docker compose --profile dev up -d ${resolvedService}`,
      );
...
devCommand
  ...
  .action(async (service, options) => {
    ...
    if (!serviceConfig) {
      printError(`Invalid service: ${service} (${resolvedService})`);
      const execable = serviceRegistry
        .getExecableServices()
        .map((s) => s.name)
        .join(", ");
