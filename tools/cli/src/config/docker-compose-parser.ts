 ...
 */
export function parseDockerCompose(projectRoot: string = process.cwd()): Map<string, ServiceConfig> {
  ...
  try {
    ...
    for (const [serviceName, serviceConfig] of Object.entries(compose.services)) {
      ...
      const containerName = generateContainerName(projectName, serviceName, profile);
      ...
      services.set(serviceName, {
        ...
        metadata: {
          ...
          volumes: serviceConfig.volumes?.map(v => typeof v === 'string' ? v : ''),
    ...
    throw new Error(`Failed to parse docker-compose.yml: ${error instanceof Error ? error.message : error}`);
  }
}
 ...
 */
function generateContainerName(project: string, service: string, profile: 'dev' | 'prod' | 'all'): string {
...
function extractPorts(service: DockerComposeService): number[] {
  ...
  return service.ports.map(portMapping => {
 ...
 */
export function getExecableServices(services: Map<string, ServiceConfig>): ServiceConfig[] {
 ...
 */
export function getRebuildableServices(services: Map<string, ServiceConfig>): ServiceConfig[] {
